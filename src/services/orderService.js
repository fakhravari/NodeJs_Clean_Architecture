...
async function update(id, data) {
  ...
  await pool
    ...
    .query(
      `UPDATE ${model.tableName} SET TotalAmount=@TotalAmount WHERE OrderID=@id`
    );
}
...
async function getAllFromViewFormatted() {
  ...
  return rows.map((r) => {
    ...
    const orderDate = r.OrderDate
      ? new Date(r.OrderDate).formatDateToYMD()
      : null;
    const total =
      r.TotalAmount != null ? Number(r.TotalAmount).toFixed(2) : "0.00";
    const jsonDetails =
      r.JsonDetails != null
        ? `N'${String(r.JsonDetails).escapeSqlString()}'`
        : "NULL";
    const datePart = orderDate ? `N'${orderDate.escapeSqlString()}'` : "NULL";
